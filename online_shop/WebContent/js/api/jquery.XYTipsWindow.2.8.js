	...
	$.extend($.XYTipsWindow,{
		...
		init: function (o){
			...
			if(o.___showbg != "" && o.___showbg == true){
				var $boxBgDom = "<div id=\"XYTipsWindowBg\" style=\"position:absolute;background:"+o.___windowBgColor+";filter:alpha(opacity=0);opacity:0;width:100%;left:0;top:0;z-index:5618\"><iframe src=\"about:blank\" style=\"width=100%;height:"+$(document).height()+"px;filter:alpha(opacity=0);opacity:0;scrolling=no;z-index:5610\"></iframe></div>";
		...
		showBox: function(o) {
			...
			var $boxDom = "<div id=\""+o.___boxID+"\" class=\"XYTipsWindow\">";
				...
				$boxDom += "<iframe src=\"about:blank\" style=\"position:absolute;left:0;top:0;filter:alpha(opacity=0);opacity:0;scrolling=no;z-index:8714\"></iframe>";
				$boxDom += "</div>";
		...
		setBoxzIndex: function (o) {
			...
			$box.die().live("click",function(){
				for(var i=0; i < $XYTipsWindowarr.length; i++){
					$XYTipsWindowarr[i].style.zIndex = 10618;
				};
				this.style.zIndex = 11208;//891208;
			});
		},
		//写入CSS样式
		addStyle : function(s) {
